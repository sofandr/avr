// Шим на частоте 18.7Khz 
// Стало не слышно писка, добавил еще 22 пикафарад кондер в цепь питания вентилятора
// Подравил, частота 9.6Mhz, fuse бит CKDIV8 установлен в 1, если 0 то 1.2Mhz
// Как то не логично для меня.., 0 устанавливает делитель частоты  а 1 снимает...
// 
//
//
//#define diff_pwm 5 // увеличиваем значение регистра увеличиваем значение регистра совпадения для ШИМ таймера, пока не достигнет максимума
#define diff_pwm 1
...
ISR(TIM0_OVF_vect)
{
	if ( state != 0 )
	{
		...
		else
		{
			...
			{
				if (OCR0A <= 250)
					OCR0A += diff_pwm; // увеличиваем значение регистра совпадения для ШИМ таймера, пока не достигнет максимума
			...
			{
				if (OCR0A >= diff_pwm)
					OCR0A -= diff_pwm; // уменьшаем значение регистра совпадения для ШИМ таймера, пока не достигнет нуля
